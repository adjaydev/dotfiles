...
if [ -f "$TOKEN_CACHE" ]; then
    ...
    if ! op signin --session="$TOKEN" --output=raw > /dev/null; then
        ...
        # TOKEN=$(rofi -dmenu -password -p "1password: "| op signin --output=raw) || TOKEN=""
				...
				# macos
				TOKEN=$(osascript -e 'Tell application "System Events" to display dialog "1passsssword:" default answer "" with icon caution with hidden answer' -e 'text returned' | op signin --account D5RGRUSNHVBO5DS3ZNMZ3KOMMM) || TOKEN=""
...
if [ -n "$TOKEN" ]; then
    ...
    if [ -z "$UUID" ] || [ "$UUID" == "null" ];then
        ...
        # TITLE=$(op list items --cache --session="$TOKEN" | jq -r '.[].overview.title' | rofi -dmenu -i) || TITLE=""
				...
				# macos
				TITLE=$(op list items --cache --session="$TOKEN" | jq -r '.[].overview.title' | osascript -e 'set text_items to paragraphs of (read (POSIX file "/dev/stdin"))' -e 'choose from list text_items with prompt "Select 1Password Entry:"' -e 'result as string') || TITLE=""
